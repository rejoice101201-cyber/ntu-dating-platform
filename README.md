...
- **多重登入**：Google OAuth、UserID（可免密碼）、Email/Password；忘記密碼寄送重設連結。
- **像素風 UI**：Press Start 2P 字體、淺色像素格背景、深色高對比按鈕。
- **探索/配對**：推薦清單、1~5 分評分（消耗體力）、標籤顯示與匹配度。
- **聊天室**：Pusher 即時訊息、時間戳、AI 開場白建議、Q&A 遊戲（發起/回答/猜測），完成或正確可獲得鑰匙。
- **解鎖照片**：照片有模糊度，透過鑰匙解鎖；支援 Vercel Blob 上傳/刪除個人照片。
- **個人檔案**：可編輯基本資料（身高、體重、職業、學校、血型、位置、Bio），設定配對偏好（性別、年齡區間），查看他人檔案與解鎖進度。
- **AI/QA/Game API**：AI coach（opening lines / topics / profile 建議）、QA 題庫、猜題/答題遊戲、照片模糊與解鎖邏輯。
