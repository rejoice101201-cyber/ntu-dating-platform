...
### 能量機制
- **每日自動補滿**：
  - 每日 06:00 UTC+8 自動補滿至能量上限（預設 100）
...
- **登入頁面** (`/auth/login`)
  ...
  - 10 分鐘內免重登檢查
- **註冊頁面** (`/auth/register`)
  - 註冊表單（email、密碼、姓名、生日、性別等）
  - 表單驗證
  - 註冊成功後自動登入
- **忘記密碼頁面** (`/auth/forgot`)
  - 輸入 email 表單
  - 發送重設連結至信箱
- **重設密碼頁面** (`/auth/reset/[token]`)
  - 輸入新密碼表單
  - 驗證 token 有效性
  - 更新密碼後自動登入

### 主要功能頁面
- **Discover 頁面** (`/discover`)
  - 顯示推薦用戶卡片
  - 顯示當前能量值
  - 1-5 分評分按鈕
  - 無名單時顯示提示
...
- **Home 頁面** (`/home`)
  - 左側：熱門主題列表（依貼文數排序）
  - 右側：配對排行榜 Top 10（含 userId、bio、模糊頭貼）
  - 中間：貼文列表（可排序、篩選）
  - 發文功能（支援圖片上傳）
- **Wall 頁面** (`/w`)
  - 顯示所有貼文（可排序、篩選）
...
- **Matches 頁面** (`/matches`)
  ...
  - 點擊進入聊天室
  - 無配對時顯示提示
- **Chat 頁面** (`/chat/[matchId]`)
  - 即時訊息顯示（Pusher + polling fallback）
  - 訊息輸入框
  - AI Coach 開場白建議（三種人設）
  - Q&A 遊戲面板
  - 解鎖進度顯示
  - 顯示對方照片（模糊等級）
- **Profile 頁面** (`/profile/[id]`)
  - 查看他人個人檔案
  - 顯示照片（模糊等級）、標籤、bio
  - 顯示解鎖進度
  - 發起配對按鈕
- **My Profile 頁面** (`/profile/me`)
  - 編輯個人資料（姓名、生日、性別、身高、位置、bio）
  - 上傳/刪除照片（Vercel Blob）
  - 設定標籤（興趣標籤）
  - 設定配對偏好（性別、年齡區間）
  - AI Coach 個人檔案優化建議
- **Search 頁面** (`/search`)
  - 搜尋主題功能（即時搜尋，300ms debounce）
  - 建立新主題功能
  - 主題列表顯示
- **Saved 頁面** (`/saved`)
  - 顯示收藏的貼文列表
