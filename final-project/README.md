# Shibuya - 「因為聊得來，才想認識你」-結合AI回覆免費社群交友平台

Deployed service 網址：  
https://ntu-dating-platform1.vercel.app/

Github Repo 網址:
https://github.com/rejoice101201-cyber/wp1141/tree/main/final-project

Demo 影片網址:
https://www.youtube.com/watch?v=3wBf5j4LiHk

FB 社團貼文網址:
https://www.facebook.com/share/p/1KYX8t3EtU/

## 專案簡介

Shibuya 是一個使用 Next.js 14 (App Router) + Prisma + PostgreSQL 打造的交友平台，我們的交友平台核心理念是
「因為聊得來，才想認識你」。

和傳統交友軟體不同，我們不是先看照片滑來滑去，
而是讓使用者先從 共同話題、價值觀與實際互動 開始認識彼此。

系統每天會透過 Gemini AI 產生今日主題，
使用者可以針對主題發文、互動，
最後再從這些真實互動中，自然發展成配對。

我們把 AI 模型塞進聊天室，解決你的社交恐懼； 我們把即時互動遊戲寫進訊息流，因為無聊是最大的罪過； 我們堅持注重個性與社群連結，因為這裡不是看臉的菜市場。
...
1. **認證系統（完整實作）**
   - Google OAuth 登入：伺服器端 token 交換、email 驗證、JWT 生成
   - Email/密碼登入：bcrypt 密碼加密、註冊流程、登入驗證
   - 忘記密碼功能：JWT token 生成、Nodemailer 郵件發送、重設密碼頁面
   - 10 分鐘內免重登：localStorage 機制、token 有效性檢查

2. **能量機制（完整實作）**
   - 每日自動補滿：`lib/energy.ts` 實作、`lastEnergyRefill` 欄位管理、UTC+8 時區處理
   - 能量獲得/消耗邏輯：發文 +10 能量、評分扣能量、Q&A 遊戲扣 5 能量
   - `clampEnergy` 函數：防止能量 < 0 的防呆機制
   - 即時同步：`refreshEnergy()` API 端點、前端能量狀態更新

3. **即時聊天系統（完整實作）**
   - Pusher 整合：即時訊息推送、頻道訂閱管理
   - Polling fallback 機制：5 秒輪詢補齊遺漏訊息
   - Optimistic 更新：前端立即顯示、後端確認機制
   - 訊息 API：`/api/chat/[matchId]/route.ts` 完整實作
   - 聊天室頁面：`app/chat/[matchId]/page.tsx` 完整開發

4. **Q&A 遊戲後端（完整實作）**
   - 遊戲 API：`/api/game/` 所有端點
   - 遊戲狀態管理：GameSession 資料庫模型設計
   - Pusher 遊戲狀態推送：即時同步雙方回答與猜測
   - 鑰匙獲得機制：遊戲完成後解鎖邏輯

5. **資料庫設計與 Prisma Schema**
   - 所有資料庫模型定義：User, Match, Post, GameSession, Message 等
   - Prisma Client 設定與優化
   - 資料庫遷移腳本
...
1. **配對系統（完整實作）**
   - Discover 頁面：`app/discover/page.tsx` 完整開發
   - 推薦算法前端整合：標籤匹配度顯示、共同興趣計算
   - 評分功能：1-5 分評分系統、即時能量扣除、移至下一位邏輯
   - 配對流程：雙方評分後自動建立 Match、配對成功提示
   - 配對限制：每日 3 次上限顯示、剩餘次數計算
   - Matches 頁面：配對列表顯示、待處理配對請求、接受/拒絕功能

2. **貼文系統（完整實作）**
   - 建立貼文：`app/home/page.tsx`、`app/w/page.tsx` 完整開發
   - 圖片上傳：Vercel Blob Storage 整合
   - 主題選擇：今日主題、使用者自創主題、主題搜尋功能
   - 貼文操作：按讚、收藏、編輯、刪除功能
   - 貼文篩選：最新/熱門排序、依主題/作者篩選
   - 貼文顯示：相對時間顯示、匿名機制、圖片預覽
   - 從貼文配對：配對請求發起功能

3. **主題系統（完整實作）**
   - 每日主題：Gemini API 整合、每日主題生成與顯示
   - 熱門主題：貼文數排序、Home 頁面左側顯示
   - 使用者自創主題（board）：主題搜尋、建立新主題、主題頁面
   - 主題搜尋：300ms debounce 即時搜尋、搜尋結果顯示

4. **AI Coach（完整實作）**
   - 三種人設：小奶狗、霸道總裁、高貴御姐的 prompt 設計
   - Gemini API 整合：開場白生成、個人化建議
   - 個人檔案優化建議：照片數量、bio 長度、標籤數量分析
   - 話題建議：根據配對對象興趣標籤生成
   - AI Coach UI：聊天室整合、開場白點擊填入功能

5. **導航系統（完整實作）**
   - 左側邊欄：`components/LeftSidebar.tsx` 完整開發
   - 底部導航：`components/Navigation.tsx` 完整開發
   - 路由管理：頁面切換邏輯、聊天頁面自動隱藏導航

6. **狀態管理與前端優化**
   - Zustand store：`store/authStore.ts` 設計與實作
   - localStorage 持久化：認證狀態保存
   - 前端錯誤處理：API 錯誤提示、用戶引導
...
1. **照片模糊與解鎖系統（完整實作）**
   - 模糊等級計算：`blurLevel = 100 - (unlockLevel * 10)` 公式實作
   - CSS `filter: blur()` 視覺效果：動態模糊等級應用
   - 解鎖機制：Q&A 遊戲完成後鑰匙獲得、解鎖進度更新
   - 照片逐漸清晰：解鎖進度與模糊等級同步更新
   - 解鎖動畫效果：照片清晰度過渡動畫

2. **排行榜系統（完整實作）**
   - 配對數 Top 10：資料庫查詢優化、排序邏輯
   - Home 頁面右側顯示：排行榜 UI 設計
   - 用戶檔案連結：點擊查看用戶詳細資訊
   - 模糊頭貼顯示：與照片模糊系統整合

3. **Q&A 遊戲前端（完整實作）**
   - 遊戲 UI：`app/chat/[matchId]/page.tsx` 中的遊戲面板
   - 題目顯示：繁體中文題目渲染、主題分類顯示
   - 回答與猜測介面：用戶輸入、答案提交
   - 遊戲進度顯示：解鎖進度條、鑰匙數量顯示
   - 題目庫管理：興趣、個性、生活方式分類
...
4. **系統測試與驗證**
   - 功能測試：各模組功能完整性驗證
   - 用戶流程測試：登入到配對到聊天的完整流程
   - Bug 發現與回報：從使用者角度測試、edge case 發現
   - 測試文件：功能 checklist、user flow 測試記錄

5. **文件與設計參考**
   - README.md 維護：專案文件更新
   - 設計參考整理：`docs/design-references/` 圖片整理
   - 專題製作心得撰寫：團隊協作經驗記錄
...
- **核心架構設計**：初期一起討論 Next.js App Router 架構、資料庫設計
- **API 設計**：RESTful API 端點規劃、請求/回應格式定義
- **UI/UX 設計**：整體頁面佈局、用戶體驗流程設計
- **程式碼審查**：互相檢查程式碼、提出改進建議
- **問題解決**：遇到技術難題時共同討論解決方案
...
## 專題製作心得
吳朋員：我們一開始其實不是要做交友平台，但做到一半突然意識到：現在的交友 App 大多都是看臉配對，但真的要把關係做穩、做得久，個性與價值觀的匹配才是關鍵，不然配對再多也只是表面。也因為這樣，我們才轉向做這種「互動型交友」——它不一定最炫，但我覺得對想認真找對象的人真的有幫助，尤其是那種想透過深度互動了解對方、但又不知道怎麼開始的人。做的過程也讓我發現：照片模糊解鎖機制其實很難，因為要把互動流程設計得有趣，但又不能太複雜。之後如果時間夠，我會想把 Q&A 遊戲的題目庫做得更豐富（例如：加入更多情境題），也補一些解鎖動畫效果，讓它更像真的能陪人從陌生到熟悉的工具。
鄧以謙：這次專案讓我最有感的是「即時通訊的穩定性」真的很重要。我們一開始只用 Pusher，但後來發現有時候會漏訊息，用戶體驗很差。後來我們加上了 polling fallback，雖然看起來是「退而求其次」的方案，但其實這種「雙重保障」的設計思維，在生產環境中真的很關鍵。另外，AI Coach 的功能也讓我學到：不是所有功能都要自己從頭做，善用 Gemini API 可以快速做出有價值的功能，重點是要把 prompt 設計好，讓 AI 產出的內容符合我們想要的風格。這次也讓我意識到：做產品不是只有「功能做完就好」，還要考慮「如果這個服務掛了怎麼辦」、「如果用戶遇到問題怎麼辦」，這些 edge case 的處理，才是真正考驗產品成熟度的地方。之後如果時間夠，我會想加強錯誤處理和用戶引導，讓整個使用流程更順暢。
蔡蔓萱：我覺得我們這次合作最有效率的地方，是一開始就把核心架構先一起想清楚，避免後面一直打掉重來。然後我們也花了不少時間去找第三方服務或現成方案，能用就先用，真的不要為了「看起來很厲害」去重複造輪子，因為做 MVP 的重點是先驗證方向對不對。後來我們分工也分得滿明確：一個人主開發、另一個人負責驗證和找問題，這個方式比我想像中好用。因為開發的人常常會對自己的流程太習慣，會忽略一些很直覺但其實會出 bug 的地方；有人專門用「使用者角度」去玩它、刻意去戳破流程，真的能很快抓到盲點。這次也讓我學到：速度快不等於亂做，有基本的分工跟檢查機制，MVP 才能快又不會太危險。下一步我覺得可以加上更明確的測試方式（像是簡單的 checklist 或 user flow 測試），不然越做越大之後，光靠人肉驗證會開始撐不住。
